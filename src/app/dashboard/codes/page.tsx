...
function ts(v: string | null | undefined): string {
  if (!v) return "-";
  return new Date(Number(v)).toLocaleDateString("ko-KR");
}
...
function InviteTab() {
  ...
  async function create() {
    ...
    if (r.ok) { setMaxUses(""); setRewardType("none"); setRewardValue(""); setCustomCode(""); load(); }
    setSubmitting(false);
  }
  ...
  return (
    <div>
      ...
      {/* Code list */}
      {codes.length === 0 ? <p style={{ color: "var(--color-muted)", textAlign: "center", padding: "32px" }}>초대 코드가 없습니다.</p> : codes.map(c => (
...
function CouponTab() {
  ...
  async function create() {
    ...
    if (r.ok) { setCode(""); setDescription(""); setDiscountValue(""); setMaxUses(""); setExpiresAt(""); load(); }
    setSubmitting(false);
  }

  async function toggle(c: Coupon) {
    await fetch(`/api/v1/codes/coupon/${c.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ active: !c.active }) });
    load();
  }
  ...
  return (
    <div>
      <div style={{ background: "var(--color-lifted)", border: "1px solid var(--color-hairline)", borderRadius: "10px", padding: "16px", marginBottom: "20px" }}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: "8px", marginBottom: "10px" }}>
          <div style={{ flex: "1 1 140px" }}><Input value={code} onChange={setCode} placeholder="코드 (자동생성 가능)" /></div>
      ...
      {coupons.length === 0 ? <p style={{ color: "var(--color-muted)", textAlign: "center", padding: "32px" }}>쿠폰이 없습니다.</p> : coupons.map(c => (
...
function LicenseTab() {
  ...
  async function create() {
    ...
    if (r.ok) { setLabel(""); setProduct(""); setMaxActivations("1"); setExpiresAt(""); setCustomKey(""); load(); }
    setSubmitting(false);
  }

  async function toggle(k: LicenseKey) {
    await fetch(`/api/v1/codes/license/${k.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ active: !k.active }) });
    load();
  }
    ...
    <div>
      <div style={{ background: "var(--color-lifted)", border: "1px solid var(--color-hairline)", borderRadius: "10px", padding: "16px", marginBottom: "20px" }}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: "8px", marginBottom: "10px" }}>
          ...
          <div style={{ flex: "1 1 140px" }}><Input value={expiresAt} onChange={setExpiresAt} placeholder="만료일" type="date" /></div>
          <div style={{ flex: "2 1 200px" }}><Input value={customKey} onChange={setCustomKey} placeholder="커스텀 키 (선택, XXXX-XXXX-XXXX-XXXX)" /></div>
        </div>
        <Btn onClick={create} disabled={submitting}>{submitting ? "생성 중..." : "생성"}</Btn>
      </div>

      {keys.length === 0 ? <p style={{ color: "var(--color-muted)", textAlign: "center", padding: "32px" }}>라이선스 키가 없습니다.</p> : keys.map(k => (
        <div key={k.id} style={{ background: "var(--color-lifted)", border: "1px solid var(--color-hairline)", borderRadius: "8px", padding: "12px 16px", marginBottom: "8px", display: "flex", alignItems: "center", gap: "10px", flexWrap: "wrap" }}>
          <code style={{ fontFamily: "monospace", fontWeight: 700, letterSpacing: "0.1em", cursor: "pointer" }} onClick={() => copyText(k.key)} title="클릭하여 복사">{k.key}</code>
          ...
          <span style={{ fontSize: "0.8rem", color: "var(--color-muted)" }}>활성화 {k.activations}/{k.max_activations}</span>
...
function OtpTab() {
  ...
  const load = useCallback(async () => {
    ...
    if (r.ok) { const d = await r.json(); setOtps(d.codes); }
  }, []);
  ...
  return (
    <div>
      ...
      {otps.length === 0 ? <p style={{ color: "var(--color-muted)", textAlign: "center", padding: "32px" }}>생성된 OTP가 없습니다.</p> : otps.slice(0, 20).map(o => (
...
function UrlTab() {
  ...
  const load = useCallback(async () => {
    ...
    if (r.ok) { const d = await r.json(); setUrls(d.urls); }
  }, []);

  useEffect(() => { load(); }, [load]);

  async function create() {
    if (submitting || !originalUrl) return;
    setSubmitting(true);
    const r = await fetch("/api/v1/codes/url", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ target_url: originalUrl, title, custom_code: customCode || undefined }),
    });
    const d = await r.json();
    if (r.ok) { setOriginalUrl(""); setTitle(""); setCustomCode(""); load(); }
    else alert(d.error || "생성 실패");
    setSubmitting(false);
  }

  async function toggle(u: ShortUrl) {
    await fetch(`/api/v1/codes/url/${u.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ active: !u.active }) });
    load();
  }
  ...
  const shortBase = typeof window !== "undefined" ? `${window.location.protocol}//go.krl.kr` : "https://go.krl.kr";
  ...
  return (
    <div>
      <div style={{ background: "var(--color-lifted)", border: "1px solid var(--color-hairline)", borderRadius: "10px", padding: "16px", marginBottom: "20px" }}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: "8px", marginBottom: "10px" }}>
          <div style={{ flex: "3 1 240px" }}><Input value={originalUrl} onChange={setOriginalUrl} placeholder="https://..." /></div>
        ...
        <Btn onClick={create} disabled={submitting || !originalUrl}>{submitting ? "생성 중..." : "단축하기"}</Btn>
      </div>

      {urls.length === 0 ? <p style={{ color: "var(--color-muted)", textAlign: "center", padding: "32px" }}>단축 URL이 없습니다.</p> : urls.map(u => (
        <div key={u.id} style={{ background: "var(--color-lifted)", border: "1px solid var(--color-hairline)", borderRadius: "8px", padding: "12px 16px", marginBottom: "8px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "10px", flexWrap: "wrap", marginBottom: "4px" }}>
            <code style={{ fontFamily: "monospace", fontWeight: 700, color: "var(--color-accent)", cursor: "pointer" }}
              onClick={() => copyText(`${shortBase}/${u.code}`)} title="클릭하여 복사">
              go.krl.kr/{u.code}
            </code>
