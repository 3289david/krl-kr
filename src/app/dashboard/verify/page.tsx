      ...
      <div style={{ display:"flex", gap:4, marginBottom:24, background:"var(--color-canvas)", borderRadius:10, padding:4 }}>
        {(["list","create","check"] as const).map(t => (
          <button key={t} onClick={() => { setTab(t); setMsg(null); setCheckResult(null); setNewRecord(null); }}
            style={{ flex:1, padding:"8px", border:"none", borderRadius:8, cursor:"pointer", fontWeight:tab===t?700:400, background:tab===t?"var(--color-surface)":"none", color:tab===t?"var(--color-ink)":"var(--color-muted)", boxShadow:tab===t?"0 1px 4px rgba(0,0,0,.1)":"none", fontSize:".875rem" }}>
            ...
            {records.map(r => (
              <div key={r.id} style={{ background:"var(--color-surface)", borderRadius:12, padding:"14px 16px", border:"1px solid var(--color-hairline)" }}>
                <div style={{ display:"flex", alignItems:"flex-start", gap:12 }}>
                  <div style={{ width:40, height:40, borderRadius:10, background:"#eff6ff", display:"flex", alignItems:"center", justifyContent:"center", fontSize:"1.3rem", flexShrink:0 }}>
                    {r.mimeType.startsWith("image/")?"🖼️":r.mimeType.startsWith("audio/")?"🎵":r.mimeType.startsWith("video/")?"🎬":"📄"}
                  </div>
        ...
        <div style={{ maxWidth:520 }}>
          {newRecord ? (
            <div style={{ background:"#f0fdf4", border:"1px solid #bbf7d0", borderRadius:12, padding:20 }}>
              <div style={{ fontWeight:700, color:"#166534", marginBottom:12, fontSize:"1rem" }}>✅ 기록이 생성되었습니다!</div>
            ...
            <div>
              <div style={{ border:"2px dashed var(--color-hairline)", borderRadius:12, padding:28, textAlign:"center", cursor:"pointer", marginBottom:16, background:createFile?"#f0fdf4":"var(--color-canvas)" }}
                ...
                {createFile
                  ? <><div style={{ fontSize:"1.5rem" }}>✅</div><div style={{ fontWeight:700, marginTop:4 }}>{createFile.name}</div><div style={{ color:"var(--color-muted)", fontSize:".8rem" }}>{fmt(createFile.size)}</div></>
                  : <><div style={{ fontSize:"2.5rem" }}>📂</div><div style={{ fontWeight:600, marginTop:8 }}>클릭하여 파일 선택</div><div style={{ color:"var(--color-muted)", fontSize:".8rem" }}>최대 500MB · 모든 파일 형식</div></>}
              </div>
      ...
      {tab === "check" && (
        <div style={{ maxWidth:520 }}>
          ...
          <div style={{ border:"2px dashed var(--color-hairline)", borderRadius:12, padding:20, textAlign:"center", cursor:"pointer", marginBottom:16, background:checkFile?"#eff6ff":"var(--color-canvas)" }}
            ...
            {checkFile
              ? <><div style={{ fontWeight:700 }}>{checkFile.name}</div><div style={{ color:"var(--color-muted)", fontSize:".8rem" }}>{fmt(checkFile.size)}</div></>
              : <><div style={{ fontSize:"2rem" }}>🔍</div><div style={{ fontWeight:600, marginTop:4 }}>검증할 파일 선택</div></>}
          </div>
          ...
          {checkResult && (
            <div style={{ background:checkResult.match?"#f0fdf4":"#fef2f2", border:`1px solid ${checkResult.match?"#bbf7d0":"#fecaca"}`, borderRadius:12, padding:16 }}>
              <div style={{ fontWeight:800, fontSize:"1.1rem", color:checkResult.match?"#166534":"#dc2626", marginBottom:8 }}>
