...
export default function TasksPage() {
  ...
  return (
    <div style={{ display: "flex", height: "calc(100vh - 54px)", overflow: "hidden" }}>
      {/* Sidebar */}
      <div style={{ width: "200px", borderRight: "1px solid var(--color-hairline)", padding: "16px 0", flexShrink: 0, overflowY: "auto" }}>
        ...
        {[{ id: null, name: "전체", color: "#6366f1" }, ...projects].map(p => (
          <div key={p.id ?? "all"} onClick={() => setSelectedProject(p.id)} style={{ padding: "8px 16px", cursor: "pointer", fontSize: "0.875rem", fontWeight: selectedProject === p.id ? 600 : 400, background: selectedProject === p.id ? "var(--color-surface-card)" : "transparent", display: "flex", alignItems: "center", gap: "8px" }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "24px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "24px" }}>
          ...
          <button onClick={() => setShowNewTask(true)} style={{ padding: "8px 16px", background: "var(--color-accent)", color: "white", border: "none", borderRadius: "8px", cursor: "pointer", fontSize: "0.875rem" }}>+ 새 할 일</button>
        </div>
          ...
          return (
            <div key={key} style={{ marginBottom: "32px" }}>
              ...
              {loading ? <div style={{ color: "var(--color-muted)", fontSize: "0.875rem" }}>로딩 중...</div> : statusTasks.length === 0 ? (
                ...
                <div key={task.id} style={{ background: "var(--color-lifted)", border: "1px solid var(--color-hairline)", borderRadius: "10px", padding: "12px 16px", marginBottom: "8px", display: "flex", alignItems: "center", gap: "12px" }}>
                  ...
                  <span style={{ fontSize: "0.75rem", padding: "2px 8px", borderRadius: "99px", background: `${PRIORITIES[task.priority]?.color}20`, color: PRIORITIES[task.priority]?.color, fontWeight: 600 }}>
                    {PRIORITIES[task.priority]?.label}
                  </span>
                  {task.status === "todo" && (
                    <button onClick={() => updateStatus(task, "in_progress")} style={{ fontSize: "0.75rem", padding: "4px 8px", background: "var(--color-surface)", border: "1px solid var(--color-hairline)", borderRadius: "6px", cursor: "pointer" }}>시작</button>
                  )}
                  <button onClick={() => deleteTask(task.id)} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--color-muted)", fontSize: "0.875rem", padding: "4px" }}>✕</button>
                </div>
              ))}
            </div>
          );
