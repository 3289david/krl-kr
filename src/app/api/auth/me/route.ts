...
export async function GET(request: NextRequest) {
  try {
    ...
    const user = await db
      .prepare("SELECT id, email, name, plan, avatar_url, created_at, totp_enabled FROM users WHERE id = ? LIMIT 1")
      .bind(session.userId)
      .first<{ id: string; email: string; name: string | null; plan: string; avatar_url: string | null; created_at: number; totp_enabled: boolean }>();
