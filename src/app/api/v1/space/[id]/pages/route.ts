...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const result = await pool.query("SELECT * FROM space_pages WHERE site_id = $1 ORDER BY sort_order, title", [id]);
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const result = await pool.query(
      "INSERT INTO space_pages (site_id, slug, title, content, is_published, sort_order, created_at, updated_at) VALUES ($1, $2, $3, '[]', FALSE, 0, $4, $4) RETURNING *",
      [id, slug, title, now]
    );
