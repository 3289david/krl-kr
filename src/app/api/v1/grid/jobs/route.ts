...
export async function POST(request: NextRequest) {
  ...
  return NextResponse.json({ job: { id, title: title.trim(), type, cpu_req: Number(cpu_req), ram_req: Number(ram_req), node_id: resolvedNodeId, status } }, { status: 201 });
}
...
export async function PATCH(request: NextRequest) {
  ...
  await db.prepare("UPDATE grid_jobs SET status = COALESCE(?, status), result = COALESCE(?, result), updated_at = ? WHERE id = ?")
    .bind(status ?? null, result ?? null, Date.now(), id).run();
