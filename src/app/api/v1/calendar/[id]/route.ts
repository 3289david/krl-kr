...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    let idx = 1;
    for (const key of ["title", "description", "location", "start_at", "end_at", "all_day", "color", "calendar_id"]) {
    ...
    const result = await pool.query(
      `UPDATE calendar_events SET ${fields.join(", ")} WHERE id = $${idx++} AND user_id = $${idx} RETURNING *`,
      values
    );
