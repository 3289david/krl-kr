...
export async function GET(
  ...
  try {
    ...
    return handleAPIError(err, "GET /api/v1/links/[id]");
  }
}
  ...
  try {
    ...
    if (data.expires_at !== undefined) {
      if (data.expires_at === null) {
        ...
        if (isNaN(ts)) return NextResponse.json({ error: "잘못된 날짜 형식입니다." }, { status: 400 });
    ...
    if (newSlug && newSlug !== existing.slug) invalidateSlug(newSlug).catch(console.warn);

    const updated = await db.prepare("SELECT * FROM links WHERE id = ? LIMIT 1").bind(id).first<Record<string, unknown>>();
    return NextResponse.json({ link: updated });
  } catch (err) {
    return handleAPIError(err, "PATCH /api/v1/links/[id]");
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    await db.prepare("DELETE FROM links WHERE id = ? AND user_id = ?").bind(id, user.id).run();
    invalidateSlug(link.slug).catch(console.warn);

    return NextResponse.json({ success: true });
  } catch (err) {
    return handleAPIError(err, "DELETE /api/v1/links/[id]");
  }
}
